...
class Sudoku(QMainWindow):
    ...
    def Check(self):
        ...
        for i in range(0,3):
            for j in range(3,3):
                if self.game[i][j].text() == self.game[j][j].text() and i != j and self.game[i][j].text() != "":
                    self.game[i][j].setStyleSheet("font-size :18px;color :white ;background-color:pink") 
        ...
        for i in range (9):
            ...
            for j in range (9):
                if numbers[j] != "0":
                    ...
                    self.game[i][j].setStyleSheet("font-size :18px; color:blue ")
    ...
    def Dark(self):
        if self.ui.rB_dark.isChecked(): 
            ...
            self.ui.setStyleSheet("background-color: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:1, stop:0 rgba(170, 255, 127, 255), stop:1 rgba(255, 255, 255, 255));")
